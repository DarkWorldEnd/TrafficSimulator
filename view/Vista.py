# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file './ui_views/Vista.ui'
#
# Created by: PyQt5 UI code generator 5.15.11
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_VistaPrincipal(object):
    def setupUi(self, VistaPrincipal):
        VistaPrincipal.setObjectName("VistaPrincipal")
        VistaPrincipal.resize(980, 757)
        VistaPrincipal.setStyleSheet("background-color:rgb(255, 255, 255)")
        VistaPrincipal.setAnimated(False)
        self.centralwidget = QtWidgets.QWidget(VistaPrincipal)
        self.centralwidget.setObjectName("centralwidget")
        self.via = QtWidgets.QLabel(self.centralwidget)
        self.via.setGeometry(QtCore.QRect(120, 50, 761, 621))
        self.via.setText("")
        self.via.setPixmap(QtGui.QPixmap("./ui_views\\../assets/Background.png"))
        self.via.setScaledContents(True)
        self.via.setObjectName("via")
        self.btn_Iniciar = QtWidgets.QPushButton(self.centralwidget)
        self.btn_Iniciar.setEnabled(True)
        self.btn_Iniciar.setGeometry(QtCore.QRect(390, 700, 93, 28))
        self.btn_Iniciar.setObjectName("btn_Iniciar")
        self.btn_Detener = QtWidgets.QPushButton(self.centralwidget)
        self.btn_Detener.setEnabled(True)
        self.btn_Detener.setGeometry(QtCore.QRect(520, 700, 93, 28))
        self.btn_Detener.setObjectName("btn_Detener")
        self.semaforo1 = QtWidgets.QLabel(self.centralwidget)
        self.semaforo1.setGeometry(QtCore.QRect(280, 330, 31, 61))
        self.semaforo1.setText("")
        self.semaforo1.setPixmap(QtGui.QPixmap("./ui_views\\../img/v.jpg"))
        self.semaforo1.setScaledContents(True)
        self.semaforo1.setWordWrap(False)
        self.semaforo1.setObjectName("semaforo1")
        self.semaforo2 = QtWidgets.QLabel(self.centralwidget)
        self.semaforo2.setGeometry(QtCore.QRect(480, 550, 31, 61))
        self.semaforo2.setText("")
        self.semaforo2.setPixmap(QtGui.QPixmap("./ui_views\\../img/r.jpg"))
        self.semaforo2.setScaledContents(True)
        self.semaforo2.setObjectName("semaforo2")
        self.flecha1 = QtWidgets.QLabel(self.centralwidget)
        self.flecha1.setGeometry(QtCore.QRect(250, 450, 61, 41))
        self.flecha1.setText("")
        self.flecha1.setPixmap(QtGui.QPixmap("./ui_views\\../img/fv.jpg"))
        self.flecha1.setScaledContents(True)
        self.flecha1.setObjectName("flecha1")
        self.flecha2 = QtWidgets.QLabel(self.centralwidget)
        self.flecha2.setGeometry(QtCore.QRect(580, 540, 51, 41))
        self.flecha2.setText("")
        self.flecha2.setPixmap(QtGui.QPixmap("./ui_views\\../img/fr.jpg"))
        self.flecha2.setScaledContents(True)
        self.flecha2.setObjectName("flecha2")
        self.peatonal2 = QtWidgets.QLabel(self.centralwidget)
        self.peatonal2.setGeometry(QtCore.QRect(590, 450, 40, 40))
        self.peatonal2.setText("")
        self.peatonal2.setPixmap(QtGui.QPixmap("./ui_views\\../assets/Pedestrian_Light_Off.png"))
        self.peatonal2.setScaledContents(True)
        self.peatonal2.setObjectName("peatonal2")
        self.peatonal3 = QtWidgets.QLabel(self.centralwidget)
        self.peatonal3.setGeometry(QtCore.QRect(590, 140, 40, 40))
        self.peatonal3.setText("")
        self.peatonal3.setPixmap(QtGui.QPixmap("./ui_views\\../assets/Pedestrian_Light_On.png"))
        self.peatonal3.setScaledContents(True)
        self.peatonal3.setObjectName("peatonal3")
        self.peatonal4 = QtWidgets.QLabel(self.centralwidget)
        self.peatonal4.setGeometry(QtCore.QRect(370, 230, 40, 40))
        self.peatonal4.setText("")
        self.peatonal4.setPixmap(QtGui.QPixmap("./ui_views\\../assets/Pedestrian_Light_Off.png"))
        self.peatonal4.setScaledContents(True)
        self.peatonal4.setObjectName("peatonal4")
        self.peatonal1 = QtWidgets.QLabel(self.centralwidget)
        self.peatonal1.setGeometry(QtCore.QRect(370, 510, 40, 40))
        self.peatonal1.setText("")
        self.peatonal1.setPixmap(QtGui.QPixmap("./ui_views\\../assets/Pedestrian_Light_On.png"))
        self.peatonal1.setScaledContents(True)
        self.peatonal1.setObjectName("peatonal1")
        VistaPrincipal.setCentralWidget(self.centralwidget)

        self.retranslateUi(VistaPrincipal)
        QtCore.QMetaObject.connectSlotsByName(VistaPrincipal)

    def retranslateUi(self, VistaPrincipal):
        _translate = QtCore.QCoreApplication.translate
        VistaPrincipal.setWindowTitle(_translate("VistaPrincipal", "Semáforo"))
        self.btn_Iniciar.setText(_translate("VistaPrincipal", "Iniciar"))
        self.btn_Detener.setText(_translate("VistaPrincipal", "Detener"))
